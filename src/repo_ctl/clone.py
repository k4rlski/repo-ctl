...
def _process_job(conn, job_id, slug, host):
    ...
    # Guardrail (b): re-probe the remote; skip if there is no repo to clone.
    gh = ls_remote_head(clone_url)
    if not gh.get("head_sha"):
        return done("skipped", "no GitHub repo to clone")

    # Clone into a temp dir SIBLING of dest (same parent -> same filesystem, so
    # the final os.replace is an atomic rename), then move it into place.
    ...
    try:
        ...
        # Success: swap the freshly-cloned tree into its final home atomically.
